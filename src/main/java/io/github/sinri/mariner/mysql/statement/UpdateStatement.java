...
public class UpdateStatement extends AbstractModifyStatement {
    ...
    public String toString() {
        ...
        sql += table;
        sql += AbstractStatement.SQL_COMPONENT_SEPARATOR + "SET " + (new MarinerHelper()).joinStringArray(assignments, ",");
        if (!whereConditionsComponent.isEmpty()) {
            sql += AbstractStatement.SQL_COMPONENT_SEPARATOR + "WHERE " + whereConditionsComponent;
        }
        if (!sortRules.isEmpty()) {
            sql += AbstractStatement.SQL_COMPONENT_SEPARATOR + "ORDER BY " + (new MarinerHelper()).joinStringArray(sortRules, ",");
        }
        if (limit > 0) {
